...
std::string evaluateToJson(const std::string & expression)
{
    ...
    if (!ok(ctx.ptr, nix_setting_set(ctx.ptr, "extra-experimental-features", "nix-command flakes"), error, "enabling flakes"))
        return failure(error);
    if (!ok(ctx.ptr, nix_setting_set(ctx.ptr, "use-sqlite-wal", "false"), error, "configuring SQLite journaling"))
        return failure(error);
    if (!ok(ctx.ptr, nix_setting_set(ctx.ptr, "fsync-metadata", "false"), error, "configuring metadata sync"))
        return failure(error);
    if (!ok(ctx.ptr, nix_setting_set(ctx.ptr, "fsync-store-paths", "false"), error, "configuring store sync"))
        return failure(error);
    ...
    if (!ok(ctx.ptr, nix_flake_settings_add_to_eval_state_builder(ctx.ptr, flakeSettings, builder), error, "registering flake primops")) {
    ...
    if (!ok(ctx.ptr, nix_eval_state_builder_load(ctx.ptr, builder), error, "loading evaluator settings")) {
        nix_flake_settings_free(flakeSettings);
        nix_eval_state_builder_free(builder);
        nix_store_free(store);
        return failure(error);
    ...
    if (!ok(ctx.ptr, nix_eval_state_builder_set_lookup_path(ctx.ptr, builder, lookupPath), error, "setting lookup path")) {
        nix_flake_settings_free(flakeSettings);
        nix_eval_state_builder_free(builder);
        nix_store_free(store);
        return failure(error);
    ...
    if (!ok(ctx.ptr, nix_expr_eval_from_string(ctx.ptr, state, jsonExpression.c_str(), ".", value), error, "evaluating expression")) {
    ...
    std::string json;
    if (!ok(ctx.ptr, nix_get_string(ctx.ptr, value, appendString, &json), error, "extracting JSON string")) {
        nix_value_decref(ctx.ptr, value);
        nix_state_free(state);
        nix_store_free(store);
        return failure(error);
...
const char * nixos_regedit_eval_nix(const char * expression)
{
...
EMSCRIPTEN_KEEPALIVE
const char * nixos_regedit_current_system()
{
    return "x86_64-linux";
}
