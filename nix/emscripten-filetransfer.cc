...
EM_JS(
    int,
    nixos_regedit_browser_fetch,
    (const char * urlPtr, int method, const char * expectedETagPtr, char ** outData, size_t * outSize, char ** outUrl, char ** outEtag, char ** outError),
    {
        const newUtf8 = value => {
        ...
        const methodName = method === 1 ? "HEAD" : method === 2 ? "PUT" : method === 3 ? "POST" : method === 4 ? "DELETE" : "GET";
        try {
            const xhr = new XMLHttpRequest();
            xhr.open(methodName, url, false);
            if (expectedETag) xhr.setRequestHeader("If-None-Match", expectedETag);
            ...
            for (let i = 0; i < size; i += 1) HEAPU8[dataPtr + i] = response.charCodeAt(i) & 0xff;
...
FileTransferResult performBrowserTransfer(const FileTransferRequest & request)
{
    if (request.method != HttpMethod::Get && request.method != HttpMethod::Head)
        throw FileTransferError(
            FileTransfer::Misc, std::nullopt, "browser evaluator only supports GET and HEAD transfers for '%s'", displayUri(request));
    ...
    if (status < 0)
        throw FileTransferError(
            FileTransfer::Misc, std::nullopt, "browser fetch failed for '%s': %s", displayUri(request), errorValue.str());

    FileTransferResult result;
    result.urls.push_back(effectiveUrl.str().empty() ? request.uri.to_string() : effectiveUrl.str());
    ...
    if (status >= 400)
        throw FileTransferError(
            status == 404 ? FileTransfer::NotFound : FileTransfer::Misc,
            result.data,
            "unable to download '%s': HTTP status %d",
            displayUri(request),
            status);
